...
while True:
    ...
    # Tampilkan bounding box jika warna terdeteksi
    if color_pred1:
        cv2.rectangle(frame, (width // 3 - 50, height // 3 - 50), (width // 3 + 50, height // 3 + 50), (0, 255, 0), 2)
        cv2.putText(frame, color_pred1, (width // 3 - 50, height // 3 - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    if color_pred2:
        cv2.rectangle(frame, (2 * width // 3 - 50, 2 * height // 3 - 50), (2 * width // 3 + 50, 2 * height // 3 + 50), (255, 0, 0), 2)
        cv2.putText(frame, color_pred2, (2 * width // 3 - 50, 2 * height // 3 - 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
